...
cont_region <- read_xlsx(here::here(
...
```{r}
#| label: wd-3-numeric-ex-1
# Lab 3, Question 5 (cleaning pipeline includes numeric filter on no_participants)
...
teacher_evals_clean = teacher_evals |>
 ...
 mutate(
   across(.cols = course_id:teacher_id, 
          .fns = ~ as.character(.x)),
   across(.cols = c(academic_degree, sex),
          .fns = ~ as.factor(.x))
 )
...
Thanksgiving <- ymd("2022-11-24", tz = "America/Los_Angeles")

suspects <- suspects |>
  filter(
    Time.Spotted %within% interval(
          Thanksgiving - weeks(5),
          Thanksgiving + weeks(5)
          )
    )
